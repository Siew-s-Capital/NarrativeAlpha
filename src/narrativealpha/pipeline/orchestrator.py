...
class PipelineOrchestrator:
    ...
    async def ingest_news(
        ...
        """Run News API ingestion."""
        queries = queries or self.config.news_queries
        max_results = max_results or self.config.news_max_results
        hours_back = hours_back or self.config.hours_back
        language = language or self.config.news_language
        domains = domains if domains is not None else self.config.news_domains

        started_at = datetime.utcnow()
        result = IngestionResult(source="news", started_at=started_at, completed_at=started_at)

        if not queries:
            result.errors.append("No News queries configured")
            result.success = False
            return result
        ...
        try:
                ...
                for query in queries:
                    try:
                        async for article in client.search_articles(
                            ...
                            inserted = self.store.store_news_article(article)
                            if inserted:
                                result.posts_stored += 1
                            else:
                                result.duplicates_skipped += 1
            ...
            result.success = False
            result.errors.append(f"News ingestion failed: {str(e)}")
